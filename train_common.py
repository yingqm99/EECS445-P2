...
def predictions(logits):
    ...
    number = logits.size(0)
    # print(logits.size())
    predicted = torch.zeros(number)
    for i in range(number):
        predicted[i] = torch.argmax(logits[i]).item()
    return predicted
    #
    # pred_class = torch.argmax(logits, 1)
    # return torch.Tensor(pred_class.float())
